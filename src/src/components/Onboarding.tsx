...
const modes: Array<{
  ...
  },
  {
    id: 'source',
    title: 'AI 입력 분석',
    description: '링크나 여행 계획을 그대로 입력하면 AI가 자동 분석합니다.',
    Icon: Film,
  },
];
        ...
        <header className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">여행을 블록으로 조립하는 MyRealTrip용 Codex Plugin</p>
            </div>
          </div>
          {onBack ? (
            <button
            ...
            {mode === 'source' ? (
              <form className="space-y-4" onSubmit={handleSourceSubmit}>
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-600">링크인지 여행 계획인지 AI가 자동으로 판단해 블록으로 변환합니다.</p>
                </div>
                <textarea
                  className="min-h-64 w-full resize-y rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none transition focus:border-myrealtrip-blue focus:ring-2 focus:ring-blue-100"
                  value={sourceContent}
                  onChange={(event) => setSourceContent(event.target.value)}
                  placeholder="YouTube 링크, 블로그 링크 또는 여행 계획을 입력해보세요."
                />
