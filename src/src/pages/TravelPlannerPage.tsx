...
function getUserMode(): UserMode {
  ...
  const pathUser = window.location.pathname.toLowerCase();

  if (queryUser === 'user2' || pathUser.includes('user2')) {
    return 'User2';
  }
...
export function TravelPlannerPage() {
  ...
  const userMode = useMemo(() => getUserMode(), []);
  ...
  const refreshSavedPlans = useCallback(async () => {
    const plans = await loadSavedPlans(userMode);
    setSavedPlans(plans);
    return plans;
  }, [loadSavedPlans, userMode]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950" data-testid="trip-editor-screen">
      ...
      {activeModal === 'analysis' ? (
        <Modal title="링크/텍스트 분석" onClose={() => setActiveModal(null)}>
          <AnalysisPanel
